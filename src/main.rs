...
fn get_authorized_key(public_key: &VerifyingKey) -> String {
    ...
    let ed25519_key = Ed25519PublicKey::try_from(public_key.as_bytes())
...
fn find_ssh_keys(regex: &Regex, args: &Args, running: Arc<AtomicBool>) {
    while running.load(Ordering::SeqCst) {
        ...
        if matched {
            ...
            let private_bytes = Ed25519PrivateKey::try_from(signing_key.to_bytes().as_slice())
                .expect("Failed to create ED25519 private key");
            let public_bytes =
                ssh_key::public::Ed25519PublicKey::try_from(verifying_key.as_bytes())
                    .expect("Failed to create ED25519 public key");
