...
fn get_authorized_key(public_key: &VerifyingKey) -> String {
    ...
    // Convert [u8; 32] to Ed25519PublicKey using From trait
    let ed25519_key: Ed25519PublicKey = (*public_key.as_bytes()).into();
...
fn find_ssh_keys(regex: &Regex, args: &Args, running: Arc<AtomicBool>) {
    while running.load(Ordering::SeqCst) {
        ...
        if matched {
            ...
            // Convert byte arrays to ssh-key types using From trait
            let private_bytes: Ed25519PrivateKey = signing_key.to_bytes().into();
            let public_bytes: Ed25519PublicKey = (*verifying_key.as_bytes()).into();
