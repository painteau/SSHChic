...
fn get_authorized_key(public_key: &VerifyingKey) -> String {
    // Create SSH public key by encoding the raw bytes directly
    let mut key_bytes = Vec::new();

    // SSH ed25519 public key format:
    // - algorithm name length (4 bytes)
    // - algorithm name "ssh-ed25519" (11 bytes)
    // - key length (4 bytes)
    // - key data (32 bytes)

    // Write algorithm name
    key_bytes.extend_from_slice(&(11u32.to_be_bytes())); // length of "ssh-ed25519"
    key_bytes.extend_from_slice(b"ssh-ed25519");

    // Write public key
    key_bytes.extend_from_slice(&(32u32.to_be_bytes())); // length of key
    key_bytes.extend_from_slice(public_key.as_bytes());

    // Encode to base64
    use base64::{engine::general_purpose, Engine as _};
    format!(
        "ssh-ed25519 {} Generated by SSHChic",
...
fn find_ssh_keys(regex: &Regex, args: &Args, running: Arc<AtomicBool>) {
    while running.load(Ordering::SeqCst) {
        ...
        if matched {
            ...
            // Generate OpenSSH private key format manually
            let public_key_str = get_authorized_key(&verifying_key);
            let private_key_str = format_openssh_private_key(&signing_key, &verifying_key);
...
/// Formats an ED25519 keypair as an OpenSSH private key
///
/// This function creates an OpenSSH-format private key from raw ED25519 key material.
/// The output is compatible with OpenSSH and can be used with ssh-agent and ssh commands.
///
/// # Arguments
///
/// * `signing_key` - The ED25519 private signing key
/// * `verifying_key` - The ED25519 public verifying key
///
/// # Returns
///
/// A `String` containing the private key in OpenSSH format
fn format_openssh_private_key(signing_key: &SigningKey, verifying_key: &VerifyingKey) -> String {
    use base64::{engine::general_purpose, Engine as _};

    let mut private_section = Vec::new();

    // Check bytes (random, but we'll use fixed for simplicity)
    let check = 0x01020304u32;
    ...
    // Public key
    write_bytes(&mut private_section, verifying_key.as_bytes());

    // Private key (64 bytes: 32 private + 32 public)
    let mut private_key_full = Vec::new();
    private_key_full.extend_from_slice(&signing_key.to_bytes());
    private_key_full.extend_from_slice(verifying_key.as_bytes());
    write_bytes(&mut private_section, &private_key_full);

    // Comment
    write_string(&mut private_section, b"Generated by SSHChic");
    ...
    // Cipher (none)
    write_string(&mut key_data, b"none");

    // KDF (none)
    write_string(&mut key_data, b"none");

    // KDF options (empty)
    write_string(&mut key_data, b"");
    ...
    write_string(&mut public_section, b"ssh-ed25519");
    write_bytes(&mut public_section, verifying_key.as_bytes());
    write_bytes(&mut key_data, &public_section);

    // Private key section
    write_bytes(&mut key_data, &private_section);
