...
fn get_authorized_key(public_key: &VerifyingKey) -> String {
    ...
    // Ed25519PublicKey::from_bytes() creates from raw bytes
    let ed25519_key = Ed25519PublicKey::from_bytes(public_key.as_bytes())
        .expect("Failed to create ED25519 public key");
...
fn find_ssh_keys(regex: &Regex, args: &Args, running: Arc<AtomicBool>) {
    while running.load(Ordering::SeqCst) {
        ...
        if matched {
            ...
            // Create Ed25519 key components using from_bytes methods
            let private_bytes = Ed25519PrivateKey::from_bytes(&signing_key.to_bytes())
                .expect("Failed to create ED25519 private key");
            let public_bytes = Ed25519PublicKey::from_bytes(verifying_key.as_bytes())
                .expect("Failed to create ED25519 public key");
            ...
            // Convert Zeroizing<String> to &str for display
            ...
            if !args.streaming {
                fs::write("id_ed25519", private_key_str.as_bytes())
